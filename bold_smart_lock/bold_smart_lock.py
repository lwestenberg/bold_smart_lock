...
class Bold:
    ...
    async def remote_activation(self, device_id: int):
        ...
        return await self._auth.request("POST", f"{API_URL}{DEVICE_SERVICE}/device/{device_id}/remote-activation")

    async def get_device_permissions(self):
        """Get all effective device permissions"""
        return await self._auth.request("GET", f"{API_URL}{DEVICE_SERVICE}/effective-device-permissions")
